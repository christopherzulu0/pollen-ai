...
export function ServiceEditorModal({ service, onClose, onSave, isSubmitting = false }: ServiceEditorModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col bg-card border border-border">
        <CardHeader className="flex flex-row justify-between items-center sticky top-0 bg-card border-b border-border z-10">
          <CardTitle className="text-foreground">{service ? "Edit Service" : "Create New Service"}</CardTitle>
          <button
            onClick={onClose}
        ...
        <CardContent className="space-y-4 p-4 sm:p-6 overflow-y-auto flex-1">
          ...
          <div className="space-y-3">
            ...
            <label className="block">
              <span className="text-sm font-medium text-foreground">Name Key (Optional)</span>
              <Input
            ...
            <label className="block">
              ...
              <select
                ...
                className="mt-1 w-full border border-border rounded-md p-2 text-sm bg-background text-foreground"
              >
            ...
            {/* Icon Selector */}
            <label className="block">
              <span className="text-sm font-medium text-foreground">Icon (Optional)</span>
              ...
              <Popover open={iconPickerOpen} onOpenChange={setIconPickerOpen}>
                <PopoverTrigger asChild>
                  <Button
                    type="button"
                    variant="outline"
                    className="w-full justify-start mt-1 bg-background border-border text-foreground"
                  >
                    {formData.icon ? (
                ...
                <PopoverContent className="w-80 p-0 z-[100] bg-popover border border-border" align="start">
                        ...
                        filteredIcons.map((iconName) => {
                          ...
                          return (
                            <button
                              ...
                              className={`
                                p-3 rounded-lg border border-border transition-colors flex flex-col items-center justify-center gap-1
            ...
            {/* Image Upload */}
            <label className="block">
              <span className="text-sm font-medium text-foreground">Service Image (Optional)</span>
              <p className="text-xs text-muted-foreground mb-1">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 min-h-[40px] p-2 border border-border rounded-md bg-muted/50">
              {formData.keyFeatures && formData.keyFeatures.length > 0 ? (
                formData.keyFeatures.map((feature, index) => (
                  <div key={index} className="bg-primary/10 text-primary border border-primary/30 px-3 py-1 rounded-full text-sm flex items-center gap-2">
                    {feature}
                    <button
                      onClick={() => removeFeature(index)}
                      className="text-primary hover:text-primary/80 font-semibold text-base leading-none"
                      type="button"
                    >
                      ×
                    </button>
            ...
            <div className="flex flex-wrap gap-2 min-h-[40px] p-2 border border-border rounded-md bg-muted/50">
              {formData.requirements && formData.requirements.length > 0 ? (
                formData.requirements.map((req, index) => (
                  <div key={index} className="bg-warning/20 text-warning border border-warning/30 px-3 py-1 rounded-full text-sm flex items-center gap-2">
                    {req}
                    <button
                      onClick={() => removeRequirement(index)}
                      className="text-warning hover:text-warning/80 font-semibold text-base leading-none"
                      type="button"
                    >
                      ×
                    </button>
