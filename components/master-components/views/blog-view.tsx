    ...
    <div className="min-h-screen bg-background text-foreground py-6 sm:py-8 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="space-y-4 sm:space-y-6">
          ...
          {/* Table Skeleton */}
          <Card className="bg-card border-border shadow-lg hidden lg:block">
      ...
      <div className="mx-auto max-w-7xl">
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
              <div className="flex-1 min-w-0">
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Blog Management</h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
              <Card className="bg-card border border-border border-l-4 border-l-primary">
                <CardContent className="p-3 sm:p-4">
                  <p className="text-xs sm:text-sm text-muted-foreground font-medium">Total Posts</p>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            ...
            <select
              ...
              className="px-3 sm:px-4 py-2 border border-border rounded-lg bg-background text-foreground text-sm sm:text-base w-full sm:w-auto flex-shrink-0"
            >
          ...
          <Card className="bg-card border border-border shadow-lg hidden lg:block overflow-hidden">
            <CardContent className="p-0">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      ...
                      <tr key={post.id} className={`border-b border-border hover:bg-muted/50 transition-colors ${idx % 2 === 0 ? 'bg-card' : 'bg-muted/20'}`}>
                        ...
                        <td className="px-4 xl:px-6 py-3 xl:py-4 text-xs xl:text-sm">
                          <Badge variant="outline" className="bg-primary/10 text-primary border-primary/30 text-xs">{post.category}</Badge>
                        </td>
                        ...
                        <td className="px-4 xl:px-6 py-3 xl:py-4 text-xs xl:text-sm">
                          <Badge variant={post.status === "published" ? "default" : "secondary"} className={`text-xs ${post.status === "published" ? "bg-success/20 text-success border-success/30" : ""}`}>
                            {post.status}
                          </Badge>
                        </td>
                        <td className="px-4 xl:px-6 py-3 xl:py-4 text-xs xl:text-sm">
                        ...
                        {/* <td className="px-4 xl:px-6 py-3 xl:py-4 text-xs xl:text-sm">
                          <div className="flex items-center gap-2">
                            <TrendingUp className="w-3 h-3 xl:w-4 xl:h-4 text-secondary" />
                            <span className="font-medium text-secondary">{post.engagement}%</span>
                          </div>
                        </td> */}
                        <td className="px-4 xl:px-6 py-3 xl:py-4 text-xs xl:text-sm flex gap-1 xl:gap-2">
                          <Button
                            ...
                            className="hover:bg-accent hover:text-accent-foreground h-7 w-7 xl:h-8 xl:w-8 p-0 flex-shrink-0"
                          >
                          ...
                          <Button 
                            ...
                            className="hover:bg-destructive/10 hover:text-destructive h-7 w-7 xl:h-8 xl:w-8 p-0 flex-shrink-0 disabled:opacity-50"
                          >
          ...
          <div className="lg:hidden space-y-3 sm:space-y-4">
            {filteredPosts.length === 0 ? (
              ...
              <Card key={post.id} className="bg-card border border-border shadow-md hover:shadow-lg transition-shadow">
                <CardContent className="p-3 sm:p-4">
                  <div className="space-y-2 sm:space-y-3">
                    ...
                    <div className="flex items-start justify-between gap-2 sm:gap-3">
                      ...
                      <div className="flex gap-1 sm:gap-2 flex-shrink-0">
                        <Button
                          ...
                          className="hover:bg-accent hover:text-accent-foreground h-7 w-7 sm:h-8 sm:w-8 p-0"
                        >
                        ...
                        <Button 
                          ...
                          onClick={() => handleDelete(post.id)} 
                          disabled={deleteMutation.isPending}
                          className="hover:bg-destructive/10 hover:text-destructive h-7 w-7 sm:h-8 sm:w-8 p-0 disabled:opacity-50"
                        >
                          {deleteMutation.isPending ? (
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                      ...
                      <Badge 
                        ...
                        className={`text-xs ${post.status === "published" ? "bg-success/20 text-success border-success/30" : ""}`}
                      >
                        {post.status}
                      </Badge>
                    </div>
