        ...
        <div className="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="text-balance font-bold tracking-tight text-foreground text-3xl sm:text-4xl md:text-5xl lg:text-6xl">
              {t.groups.title_prefix}{" "}
              <span className="bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent">
                {t.groups.title_highlight}
              </span>
            </h1>
            ...
            <div className="mx-auto mt-8 sm:mt-12 lg:mt-16 grid max-w-4xl grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 px-4">
              <div className="rounded-xl border border-border/50 bg-gradient-to-br from-card to-muted/30 p-4 sm:p-6 shadow-md backdrop-blur-sm">
                ...
                <div className="mt-1 text-xs sm:text-sm font-medium text-muted-foreground">{t.groups.active_groups}</div>
              </div>
              ...
              <div className="rounded-xl border border-border/50 bg-gradient-to-br from-card to-muted/30 p-4 sm:p-6 shadow-md backdrop-blur-sm">
                ...
                <div className="mt-1 text-xs sm:text-sm font-medium text-muted-foreground">{t.groups.total_members}</div>
              </div>
              ...
              <div className="rounded-xl border border-border/50 bg-gradient-to-br from-card to-muted/30 p-4 sm:p-6 shadow-md backdrop-blur-sm">
                ...
                <div className="mt-1 text-xs sm:text-sm font-medium text-muted-foreground">{t.groups.total_saved}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
            <div className="flex items-center gap-2 text-xs sm:text-sm">
              <span className="font-medium text-foreground">
                {sortedGroups.length} {sortedGroups.length === 1 ? t.groups.group_one : t.groups.group_many}
              </span>
              {sortedGroups.length < groups.length && (
                <span className="text-muted-foreground hidden sm:inline">({t.groups.filtered_from} {groups.length})</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <Select value={sortBy} onValueChange={(value) => setSortBy(value as SortOption)}>
                <SelectTrigger className="h-9 sm:h-10 flex-1 sm:w-[180px] rounded-lg border-border/60 bg-card text-sm text-foreground">
                  <SelectValue placeholder={t.groups.sort_by} className="text-foreground" />
                </SelectTrigger>
                <SelectContent className="bg-card border-border">
                  <SelectItem value="recommended" className="text-foreground">{t.groups.recommended}</SelectItem>
                  <SelectItem value="members-high" className="text-foreground">{t.groups.most_members}</SelectItem>
                  <SelectItem value="members-low" className="text-foreground">{t.groups.least_members}</SelectItem>
                  <SelectItem value="contribution-high" className="text-foreground">{t.groups.highest_contribution}</SelectItem>
                  <SelectItem value="contribution-low" className="text-foreground">{t.groups.lowest_contribution}</SelectItem>
                  <SelectItem value="interest-high" className="text-foreground">{t.groups.highest_interest}</SelectItem>
                  <SelectItem value="newest" className="text-foreground">{t.groups.newest}</SelectItem>
                </SelectContent>
              </Select>
            ...
            {sortedGroups.length === 0 ? (
              <div className="flex min-h-[400px] sm:min-h-[500px] items-center justify-center rounded-2xl sm:rounded-3xl border-2 border-dashed border-border/50 bg-gradient-to-br from-muted/30 to-muted/10 p-8">
                <div className="max-w-md text-center">
                  ...
                  <h3 className="text-2xl sm:text-3xl font-bold text-foreground">{t.groups.no_groups}</h3>
                  <p className="mt-3 text-base sm:text-lg text-muted-foreground px-4">
                    {t.groups.no_groups_hint}
                  </p>
                  <Button
                    size="lg"
