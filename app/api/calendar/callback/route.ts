...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state) {
    return NextResponse.redirect(
      new URL("/support/settings?error=missing_params", request.url)
    );
  }
  ...
  try {
    ...
    if (Date.now() - stateData.timestamp > 5 * 60 * 1000) {
      return NextResponse.redirect(
        new URL("/support/settings?error=state_expired", request.url)
      );
    }
    ...
    if (stateData.userId !== userId) {
      return NextResponse.redirect(
        new URL("/support/settings?error=state_mismatch", request.url)
      );
    }
  } catch {
    return NextResponse.redirect(
      new URL("/support/settings?error=invalid_state", request.url)
    );
  }
  ...
  try {
    ...
    if (existingAccount) {
      ...
      return NextResponse.redirect(
        new URL("/support/settings?success=account_updated", request.url)
      );
    }
    ...
    return NextResponse.redirect(
      new URL("/support/settings?success=account_connected", request.url)
    );
  } catch (error) {
    console.error("OAuth callback error:", error);
    return NextResponse.redirect(
      new URL("/support/settings?error=oauth_failed", request.url)
    );
  }
}
