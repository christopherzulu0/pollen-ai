...
import { borrowFromAave, getSigner, TOKENS } from "@/lib/aave-helper"
...
export async function POST(request: Request) {
  try {
    ...
    const { asset, amount, interestRateMode } = body
    ...
    const rateMode = interestRateMode || 2 // Default to variable rate
    console.log(`Mock borrow: ${amount} ${asset} at rate mode ${rateMode} for user ${user.id}`)
    ...
    await prisma.transaction.create({
      data: {
        ...
        status: "COMPLETED",
        userId: user.id,
        description: `[MOCK] Borrowed ${amount} ${asset} from Aave`,
      },
    })
    ...
    return NextResponse.json({
      ...
      message: `[DEMO MODE] Successfully borrowed ${amount} ${asset}`,
      note: "This is a mock transaction. Aave V3 is not yet deployed on Celo.",
    })
  } catch (error) {
    console.error("Aave borrow error:", error)
    return NextResponse.json(
      {
        error: "Failed to borrow funds",
