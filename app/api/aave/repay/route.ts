...
import { repayLoan, getSigner, TOKENS } from "@/lib/aave-helper"
...
export async function POST(request: Request) {
  try {
    ...
    const { asset, amount, rateMode } = body
    ...
    const mode = rateMode || 2 // Default to variable rate
    console.log(`Mock repay: ${amount} ${asset} at rate mode ${mode} for user ${user.id}`)
    ...
    await prisma.transaction.create({
      data: {
        ...
        status: "COMPLETED",
        userId: user.id,
        description: `[MOCK] Repaid ${amount} ${asset} to Aave`,
      },
    })
    ...
    return NextResponse.json({
      ...
      message: `[DEMO MODE] Successfully repaid ${amount} ${asset}`,
      note: "This is a mock transaction. Aave V3 is not yet deployed on Celo.",
    })
  } catch (error) {
    console.error("Aave repay error:", error)
    return NextResponse.json(
      {
