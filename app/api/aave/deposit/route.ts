...
import { depositCollateral, getSigner, TOKENS } from "@/lib/aave-helper"
...
export async function POST(request: Request) {
  try {
    ...
    console.log(`Mock deposit: ${amount} ${asset} for user ${user.id}`)
    ...
    await prisma.transaction.create({
      data: {
        ...
        status: "COMPLETED",
        userId: user.id,
        description: `[MOCK] Deposited ${amount} ${asset} as collateral on Aave`,
      },
    })
    ...
    return NextResponse.json({
      ...
      message: `[DEMO MODE] Successfully deposited ${amount} ${asset}`,
