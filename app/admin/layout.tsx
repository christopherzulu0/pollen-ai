  ...
  return (
    <div className="flex items-center gap-4 ml-auto ">
      <div className="hidden md:flex items-center px-3 py-2 bg-background/50 border border-border/40 rounded-lg hover:bg-background/70 transition-colors">
        ...
        <input type="text" placeholder="Search..." className="ml-2 bg-transparent outline-hidden text-sm placeholder-muted-foreground w-40" />
      </div>
      ...
      <div className="w-8 h-8 rounded-full bg-linear-to-br from-primary to-secondary cursor-pointer hover:shadow-lg transition-all"></div>
    </div>
  )
}
      ...
      <div className={`${_geistSans.className} bg-background text-foreground min-h-screen`}>
        <SidebarProvider>
          ...
          <div className="flex-1 flex flex-col w-full">
            <header className="border-b border-border/40 bg-linear-to-r from-background/95 to-background/80 backdrop-blur-xl supports-backdrop-filter:bg-background/60 sticky top-0 z-40">
            ...
            <main className="flex-1 bg-linear-to-br from-background via-background to-background/50">{children}</main>
          </div>
        </SidebarProvider>
      </div>
    </AdminAccessGuard>
