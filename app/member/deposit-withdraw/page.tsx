...
import CeloDeposit from '@/components/celo/celo-deposit'
...
export default function DepositWithdraw() {
  ...
  useEffect(() => {
    const fetchBalances = async () => {
      try {
        ...
        toast.error(error instanceof Error ? error.message : "Failed to fetch balances and savings goals")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleDeposit = async (amount: string, account: string, momoNumber: string) => {
    ...
    try {
      ...
      toast.success(`$${amount} has been ${isGroup ? 'contributed to' : 'deposited to'} your ${account} account.`, {
        description: "Transaction Successful"
      })
    } catch (error) {
      console.error('Transaction error:', error)
      toast.error(error instanceof Error ? error.message : "There was an error processing your transaction. Please try again.")
    } finally {
  ...
  const handleWithdraw = async (amount: string, account: string, momoNumber: string) => {
    ...
    try {
      ...
      toast.success(`$${amount} has been withdrawn from your ${account} account.`, {
        description: "Withdrawal Successful"
      })
    } catch (error) {
      console.error('Withdrawal error:', error)
      toast.error(error instanceof Error ? error.message : "There was an error processing your withdrawal. Please try again.")
    } finally {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">Deposit & Withdraw</h1>
